...
import { View, Text, TouchableOpacity, StyleSheet, Modal, ActivityIndicator, FlatList ,Alert} from 'react-native';
...
export default function BookingItem({ item }) {
    ...
    if (store == null) return <ActivityIndicator size="small" color="#0000ff" />;
    return (
        <View>
            <TouchableOpacity
                onPress={
            ...
            >
                <View style={styles.book}>
                    <View style={styles.date}>
                        ...
                        <Text>
                            {item.listdetail[0].date}
                        </Text>
                    </View>
                    <View style={{ width: 1, height: '80%', backgroundColor: '#ccc', marginVertical: '2%', marginHorizontal: '3%' }}></View>
                    <View style={styles.location}>
                        <Text style={{ fontSize: 20 }}>
                            {item.listdetail[0].nameservice}
                        </Text>
                        ...
                        <Text style={{ color: item.status == 1 ? '#FF9100' : 'red' }}>
                    ...
                    <View style={styles.modalView}>
                        <View>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: '3%' }}>
                                ...
                                <Text style={{ fontSize: 18, marginLeft: '3%', color: item.status == 1 ? '#FF9100' : 'red' }}>
                                    {item.status == 1 ? 'Chờ xử lý' : 'Huỷ'}
                                </Text>
                            </View>
                        ...
                        <View >
                            <View style={styles.price}>
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 18, color: '#535353' }}>Tổng tiền</Text>
                                </View>
                                <View>
                                    <Text style={{ fontSize: 18, color: '#FC6011' }}>{item.total}.000</Text>
                                </View>
                            </View>
                        </View>
                        <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center', marginTop: '3%' }}
                            onPress={() => {
                                    item.status = 2;
